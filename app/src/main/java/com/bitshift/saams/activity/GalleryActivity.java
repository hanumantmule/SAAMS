...
import com.bitshift.saams.adapter.GalleryAdapter;
...
public class GalleryActivity extends AppCompatActivity {
    ...
    RecyclerView gallery_details_recyler_view;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        gallery_details_recyler_view = findViewById(R.id.recyclerView);
        galleryArrayList = new ArrayList<>();

        gallery_details_recyler_view.setLayoutManager(new LinearLayoutManager(this));
        gallery_details_recyler_view.setNestedScrollingEnabled(false);
    ...
    public void getGalleryData() {
                        ...
                        if (objectbject.has(Constant.DATA))
                        {
                            ...
                            if (jsonArray != null && jsonArray.length() > 0) {
                                for (int i = 0; i < jsonArray.length(); i++) {
                                    ...
                                    Gallery s = gson.fromJson(jsonObject.toString(), Gallery.class);
                                    galleryArrayList.add(s);
                                }
                                ...
                                if (galleryArrayList.size() > 0) {
                                    ...
                                    mShimmerViewContainer.setVisibility(View.GONE);
                                    mShimmerViewContainer.stopShimmer();
                                    mAdapter = new GalleryAdapter(activity, galleryArrayList);
                                    gallery_details_recyler_view.setLayoutManager(new LinearLayoutManager(activity));
                                    gallery_details_recyler_view.setItemAnimator(new DefaultItemAnimator());
                                    gallery_details_recyler_view.setAdapter(mAdapter);
        ...
        }, this, Constant.GET_GALLERY_URL + session.getData(Constant.ID), params, false);
    }
    @Override
